...
class AnomalyDetector:
    ...
    AVAILABLE_SCORERS: Dict[str, Tuple[str, bool]] = {
        ...
        "gmm":  ("compute_gmm",         True),   # preprocessed
    ...
    DEFAULT_ENABLED: List[str] = ["gwrp", "knn", "ae"]
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _preprocess(X_train: np.ndarray, X_test: np.ndarray, path):
        """
    ...
    def _score_one_path(self, path: str, machine: str) -> Dict[str, np.ndarray]:
        ...
        if needs_pp:
            X_train_pp, X_test_pp = self._preprocess(X_train_raw, X_test_raw, path)
        else:
        ...
        with ThreadPoolExecutor(max_workers=len(self.enabled_scorers)) as pool:
            ...
            for fut in as_completed(futures):
                ...
                if scorer_name == "ae":
                    raw_scores[scorer_name] = (fut.result() - 0.0207) / 0.0051
                elif scorer_name == "gwrp":
                    raw_scores[scorer_name] = (fut.result() - 0.0119) / 0.0034
                elif scorer_name == "knn":
                    raw_scores[scorer_name] = (fut.result() - 0.0963) / 0.0256
                elif scorer_name == "gmm":
                    raw_scores[scorer_name] = (fut.result() + 13.33 ) / 83.25
    ...
    def run(self) -> Dict:
        ...
        for machine in machines:
            ...
            print(f"  Min-pooling selections for {machine}:")
            for idx, count in zip(unique_indices, counts):
                print(f"    {scorer_names[idx].upper():>6s}: {count} samples")
